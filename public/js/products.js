...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🔹 keyword ban đầu (khi chuyển từ trang chủ sang)
    const initialKeyword  = window.initialProductKeyword || "";
    ...
    const applyFilters = () => {
        const keywordNorm = normalizeText(state.search || "");

        let filtered = items;

        // 1. Lọc theo search (bỏ dấu)
        if (keywordNorm) {
            filtered = filtered.filter((el) => {
                const title = el.dataset.title || "";
                const titleNorm = normalizeText(title);
                return titleNorm.includes(keywordNorm);
            });
        }
        ...
        // 5. Gắn lại DOM + show theo perPage
    ...
    // ===== KHỞI TẠO =====
    // Nếu có keyword từ URL (đi từ trang chủ sang), set vào input & lọc luôn
